...
`;
  ...
  const whatsappNumber = "2348088327475";
...
let currentSlide = 0;
let slideInterval = setInterval(nextSlide, 5000); // 5s interval
...
// Update dot on auto-slide
function updateDots() {
  dots.forEach((dot) => dot.classList.remove("active"));
  dots[currentSlide].classList.add("active");
}

// Modify nextSlide and prevSlide to also update dots
function nextSlide() {
  slides[currentSlide].classList.remove("active");
  currentSlide = (currentSlide + 1) % slides.length;
  slides[currentSlide].classList.add("active");
